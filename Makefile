...
check:
	...
	radon hal src > /tmp/radon_data && python -c "file = open('/tmp/radon_data', 'r'); data = file.read(); file.close(); lines = data.replace('\n    ', ' ').split('\n'); lines = [line for line in lines if 'bugs:' in line and float(line.split('bugs:')[-1].split()[0]) > 0.1]; file = open('/tmp/radon_output', 'w'); file.write('\n'.join(lines).strip()); file.close()" ; cat /tmp/radon_output ; [ ! -s /tmp/radon_output ] || (echo "Radon Halstead score too high" && exit 1)
	pyflakes .
	pyright
